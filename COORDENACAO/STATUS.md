...
| `checkpoint/duvidas-respostas-claude` | Claude Empresa + Claude Code | Cria `PROJETOS/CLIENTES/DUVIDAS E RESPOSTAS CLAUDE/` — canal de conversa por thread (um arquivo por cliente/ambiente) entre as duas instâncias. `JANAINA_COZINHA.md` já tem 2 rodadas (Dúvida/Resposta 01 e 02, incluindo discrepância de leitura da prancha 31 ainda não resolvida) + 8 dúvidas históricas backfilled; `JANAINA_ILHA.md` com 2 históricas. Anexos reais (`ANEXOS_JANAINA_COZINHA/`) copiados pro repo pra destravar o Claude Code sem depender de USB. | EM_PROGRESSO | (este commit) |
