...
| `checkpoint/claude-empresa-cassio-conhecimento` | Claude Empresa | Cria `COORDENACAO/CLAUDE_EMPRESA/` — pasta de handoff da instância que roda na empresa (papel Claude + Janaína, auditora). Primeiro registro: pipeline de Relatório de Conhecimento do projeto Cassio Fernandes (ANTES/DEPOIS/CONHECIMENTO + relatório pro líder de marcenaria + relatório de inconsistências). Trabalho de auditoria de cliente, não código de plataforma. | FROZEN | (este commit) |
| `checkpoint/duvidas-respostas-claude` | Claude Empresa + Claude Code | Cria `PROJETOS/CLIENTES/DUVIDAS E RESPOSTAS CLAUDE/` — canal de conversa por thread (um arquivo por cliente/ambiente) entre as duas instâncias. `JANAINA_COZINHA.md` já tem 4 rodadas (discrepância da prancha 31 segue bloqueante, z=0=chão-acabado resolvido em H9) + 8 dúvidas históricas backfilled; `JANAINA_ILHA.md` com 2 históricas; novo `REGRAS_CONSTRUTIVAS_GERAIS.md` consolidando regras recorrentes (menor medida, z=0, posição positiva, descontos de profundidade, gap zero). Anexos reais (`ANEXOS_JANAINA_COZINHA/`) copiados pro repo. | EM_PROGRESSO | (este commit) |
